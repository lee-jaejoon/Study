...
* 모든 물리학의 관측가능량(Observable), 예를 들면 $A$, 에는 다음을 만족하는 Operator $\hat A$가 존재한다.
  ...
  * $\varphi$는 위치값을 input(3차원 공간이라면 $\mathbb R^3$)으로 받는 complex-valued function이다.
...
* Free particle, 즉 'potential energy가 달라지는 곳에 있지 않은 입자'라고 가정한다면,
...
* 이 time-dependent Schr&ouml;dinger equation에 대한 solution은 separation of variables 테크닉을 이용해 얻을 수 있다.
  ...
  * 이에 대해 time-dependent Schr&ouml;dinger equation의 eigensolution은 아래와 같다.
    $$
    \psi_n (\mathbf r, t) = A \varphi_n(\mathbf r) \exp\left(- \frac{i E_n t}{\hslash}\right)
    $$
...
### 3.5.1. Hamiltonian이 time-independent인 경우

#### Time evolution operator의 도출

* 아래와 같이 time-dependent Schr&ouml;dinger equation에 대한 initial-value problem을 생각해보자.
  
* State function의 initial value $\psi(\mathbf r, 0)$이 주어졌을 때, $\psi(\mathbf r, t)$를 구하라.
  
* 먼저 아래와 같이 time-dependent Schr&ouml;dinger equation을 정리한다.
  $$
  \begin{align*}
  i \hslash \frac{\partial}{\partial t} \psi(\mathbf r, t) = \hat H \psi(\mathbf r, t) \\
  \frac{\partial}{\partial t} \psi(\mathbf r, t) + \frac{i \hat H}{\hslash} \psi(\mathbf r, t)= 0
  \end{align*}
  $$

* 이제 양변에 integrating factor ${\hat U}^{-1}$를 곱한다.

  * 이는 operator에 대한 function인데, 이 역시 operator가 된다.
  * Operator의 exponential은 아래와 같이 Taylor series expansion으로 둘 수 있다.

  $$
  {\hat U}^{-1} = \exp \left( \frac{it\hat H}{\hslash}\right) = 1 + \frac{it \hat H}{\hslash} +\frac{1}{2!}\left(\frac{it \hat H}{\hslash}\right)^2 + \cdots = \sum_{j=0}^{\infty} \frac{1}{j!} \left(\frac{it\hat H}{\hslash} \right)^j
  $$

  * 엄밀하게는 이것이 잘 정의되는지 확인하고 넘어가야 함.
    * operator가 bounded이면 taylor series 꼴의 exponential operator가 정의됨.
    * operator가 self-adjoint면 위 exponential operator가 잘 정의되는 analytic vectors가 dense하게 존재함.
    * https://physics.stackexchange.com/questions/574621/a-rigorous-definition-of-the-exponential-of-an-operator-in-qm
  * 양변에 위 integrating factor를 곱하면 아래와 같다.

  $$
  {\hat U}^{-1}\left[ \frac{\partial}{\partial t} \psi(\mathbf r, t) + \frac{i \hat H}{\hslash} \psi(\mathbf r, t)\right]= 0 \\
  \implies \frac{\partial}{\partial t}\left[ \exp \left( \frac{it\hat H}{\hslash}\right) \psi(\mathbf r ,t )\right]= 0
  $$

  * 이를 time interval $(0,t)$에 대해 적분하면,
    $$
    \exp \left( \frac{it\hat H}{\hslash}\right) \psi(\mathbf r ,t ) -  \psi(\mathbf r ,0 ) = 0 \\
    \exp \left( \frac{it\hat H}{\hslash}\right) \psi(\mathbf r ,t ) =  \psi(\mathbf r ,0 )
    $$

  

* 따라서 다음과 같이 time evolution operator $\hat U = \exp (-it \hat H/\hslash)$을 얻게 된다.
  $$
  \psi(\mathbf r ,t ) =  \exp \left( -\frac{it\hat H}{\hslash}\right)\psi(\mathbf r ,0 )
  $$

  * 여기서는 아래와 같은 사실을 사용하였다.

  $$
  \hat U \hat U^{-1} =\exp \left( -\frac{it\hat H}{\hslash}\right)\exp \left( \frac{it\hat H}{\hslash}\right) = \hat I \quad \text{ : identity operator}
  $$

  * Taylor expansion으로 operator의 exponential이 잘 정의된다고 한다면, 아래와 같이 위 사실을 보일 수 있음.

    * $\hat U \hat U^{-1}$을 Taylor expansion으로 풀어서 적으면 아래와 같다.

    $$
    \begin{align*}
    \hat U \hat U^{-1} \psi &=\exp \left( -\frac{it\hat H}{\hslash}\right)\left\{ \exp \left( \frac{it\hat H}{\hslash}\right)  \psi \right\} \\
    &= \sum_{\ell=0}^\infty  \frac{1}{\ell!} \left(-\frac{it\hat H}{\hslash} \right)^\ell \left[ \sum_{j=0}^\infty \frac{1}{j!} \left(\frac{it\hat H}{\hslash} \right)^j \psi\right] \\
    &= \sum_{\ell=0}^\infty \sum_{j=0}^\infty (-1)^\ell \frac{1}{\ell!} \frac{1}{j!} \frac{i^{\ell+j }t^2}{\hslash^2} \hat H^{\ell+j}\psi \\ 
    &= \psi+\sum_{k=1}^\infty a_k \hat H^{k}\psi\\
    \end{align*}
    $$

    * 이때 Taylor coefficient $a_k$는 binomial expansion을 이용하면 아래와 같이 $0$이 됨을 알 수 있다.

    $$
    \begin{align*}
    a_k &= \sum_{\ell=0}^k \frac{1}{\ell!} \frac{1}{(k-\ell)!} (-1)^\ell  \frac{i^k t^2}{\hslash^2}\\
    &= \frac{i^k t^2}{\hslash^2 k!} \sum_{\ell=0}^k {k \choose \ell} (-1)^\ell = 0
    \end{align*}
    $$

    * 따라서 $\hat U \hat U^{-1} = \hat I$이 만족한다



#### Example : Section 3.1. continued

* Initial state $\psi(\mathbf r, 0)$를 $\hat H$의 한 eigenstate $\varphi_n$으로 둔다면,
  $$
  \psi(\mathbf r, 0) = \varphi_n (\mathbf r) \\
  \hat H \varphi_n = E_n\varphi_n
  $$

  * 이때 time $t$의 state는 다음과 같이 구할 수 있다.
    * **General self-adjoint operator에 대한 spectral theorem 더 공부해야 함.**

  $$
  \begin{align*}
  \psi(\mathbf r ,t ) &= \exp \left( -\frac{it \hat H}{\hslash}\right) \varphi_n(\mathbf r) = e^{-i\omega_n t}\varphi_n(\mathbf r) \\
  \hslash \omega_n &= E_n
  \end{align*}
  $$

* 어떤 observable $A$에 대한 expectation은 time evolution에 의해 어떻게 변할까?

  * Initial time에서의 expectation은 다음과 같다. 

  $$
  \begin{align*}
  \langle A \rangle_{t=0} &= \int \psi^\ast(\mathbf r, 0) \hat A \psi(\mathbf r, 0) d\mathbf r \\
  &= \int \varphi_n^\ast \hat A \varphi d\mathbf r \\
  \end{align*}
  $$

  * Time $t > 0$에서의 system의 state에 대한 expectation은 다음과 같다.

  $$
  \begin{align*}
  \psi(\mathbf r ,t ) &= e^{-i\omega_n t}\varphi_n(\mathbf r) \\
  \langle A \rangle_t &=\int \psi^\ast(\mathbf r, t) \hat A \psi(\mathbf r, t) d\mathbf r \\
  &= e^{+i\omega_n t}e^{-i\omega_n t}\int \varphi_n^\ast \hat A \varphi d\mathbf r \\
  &= \int \varphi_n^\ast \hat A \varphi d\mathbf r \\
  \implies  \langle A \rangle_t &= \langle A \rangle_{t=0} 
  \end{align*}
  $$

  * 즉 System의 state가 Hamiltonian의 eigenstate라면, 모든 observable에 대한 expectation은 시간에 대해 불변이다.
    * 이때문에 Hamiltonian의 eigenstate를 **stationary state**라고 부른다.

* Operator의 eigenfunction들은 system의 state function과 어떤 관계가 있을까?

  * 두 번째 가정에 의해 observable $A$의 ideal measurement에 의해, system의 state는 관측된 $A$의 값에 해당하는 $\hat A$의 eigenstate로 결정된다.

  * 따라서 관측 직후의 system의 state function은 $\hat A$의 eigenstate가 되고, 이 state function은 이후 시간이 흐름에 따라 evolve한다.
    $$
    \psi(\mathbf r ,t ) =  \exp \left( -\frac{it\hat H}{\hslash}\right)\psi(\mathbf r ,0 )
    $$
...
### 3.5.2. Hamiltonian이 time-dependent인 경우

* Exercise 3.18

* 앞서 3.5.1.에서 확인한 예시는 Hamiltonian이 time $t$에 대하여 불변인 경우의 예시였다.

* 만약 Hamiltonian이 time $t$에 대한 explicit function이라면 initial value problem의 solution은 어떻게 달라질까?

  * Hamiltonian operator가 아래와 같이 time $t$에 depend한다. ($\hat H(t)$)

  $$
  \frac{\partial}{\partial t} \psi(\mathbf r, t) + \frac{i \hat H(t)}{\hslash} \psi(\mathbf r, t)= 0
  $$
...
* 앞선 Chapter 3에서는 1차원 공간 상의 free-particle에 대한 양자역학 문제를 풀어보았다.
  * Free-particle Hamiltonian은 continuous한 eigenvalue의 spectrum을 생성했다. $E_k = \hslash^2 k^2/2m$
  * 또한 그때 eigenfunction은 $\varphi_k = A \exp(ikx)$가 되었다.

* 이 장에서는 아래와 같은 One-dimensional "box"를 고려하고자 한다.
  * 두 개의 impenetrable wall 사이에 달린 줄 위를 움직이는 mass $m$의 입자.
  * 줄은 무한히 얇고, 마찰력이 없다.
  * 두 벽 사이의 거리는 $a$이다.

<img src="images/1dbox.png" alt="image-20220222234311366" style="zoom:80%;" />
